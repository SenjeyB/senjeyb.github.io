...
public:
    ...
    void updatePlayer(int64_t steamid, const std::string& name, int newScore, int rank) {
        ...
        if (exists) {
            std::string updateQuery = "UPDATE players SET name = ?, score = ?, rank = ? WHERE id = ?;";
            ...
            std::string insertQuery = "INSERT INTO players (id, name, score, rank) VALUES (?, ?, ?, ?);";
